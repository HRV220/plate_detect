...
Эта команда прочитает `Dockerfile` и соберет самодостаточный образ вашего приложения со всеми зависимостями. Она одинакова для всех операционных систем.
...
Эта команда запустит ваш сервис в фоновом режиме. Выберите команду, соответствующую вашей операционной системе.

---

#### **Для Linux и macOS (в терминале bash/zsh):**

```bash
docker run -d -p 8000:8000 \
  --name plate-cover-container \
  -v "$(pwd)/tasks_storage":/app/tasks_storage \
  plate-cover-service
```
...
#### **Для Windows (в старой командной строке, cmd.exe):**
...
- `-v ...`: монтирование локальной папки `tasks_storage` внутрь контейнера. Это ключевой шаг, который позволяет сохранять результаты обработки на вашем компьютере, а не терять их при удалении контейнера.

_(Примечание: `\`, `` ` `` и `^` — это символы переноса строки для удобства чтения в разных оболочках. Вы можете написать всю команду в одну строку без них.)_
